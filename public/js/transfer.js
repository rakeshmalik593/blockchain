...
const getUserAccountBalance = async()=>{
    var balanceInWei;
   await window.web3.eth.userAccountBalance(sessionStorage.getItem('loggedAccount'))
  ...
  const sendtoAnyAddress = async () => {
    ...
    await window.contract.methods.sendAmount(address, amount).send({ from: account });
}
...
const depositAmount = async()=> {
  ...
  await window.contract.methods.deposit().send({from:localStorage.getItem('account_address')}) 
